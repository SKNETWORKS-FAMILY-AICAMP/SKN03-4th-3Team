...
st.title("오늘이 생일인 유명인 알려주는 챗봇")

page_key = "langchain_chatbot"

#메세지를 저장 
messages = {"role":"", "content":""}
# role -> user(사용자) / assistant(AI)

if "{page_key}_messages" not in st.session_state:
...
if prompt:
    ...
    if message:
        ...
        st.session_state[f"{page_key}_messages"].append(message)

        # 챗봇 답변 
        with st.chat_message(CHATBOT_ROLE.assistant.name):
            # assistant_response = response_from_llm(prompt)
            # st.markdown(assistant_response)
            assistant_response = st.write_stream(response_from_llm(prompt=prompt, message_history=st.session_state[f"{page_key}_messages"]))
