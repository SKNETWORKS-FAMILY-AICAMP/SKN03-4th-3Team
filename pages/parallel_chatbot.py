...
# def app():
#     st.title("paralled 형 쳇봇")
    # st.write("원하는 주식 회사 입력.")
    # if st.button("lambda 형 쳇봇 페이지로 이동"):
    #     st.session_state["page"] = "lambda_chatbot"
    #     st.rerun()
...
from langchain_my.chain import create_llm, create_chain_parallel
# from langchain_my.memory import chat_with_memory
        ...
        if prompt.isdigit():
            ...
            if message:
                ...
                with st.chat_message(CHATBOT_ROLE.assistant.name):
                    assistant_response = st.write(create_chain_parallel(prompt))
